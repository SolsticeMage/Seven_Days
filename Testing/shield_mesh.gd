...
func _ready():
	...
	#	Triangle's visible face is the side with vertices in clockwise order
...
func get_vertex_normal(vertex:int):
	...
	var array_index = mesh_data[ArrayMesh.ARRAY_INDEX]
	var array_vertex = mesh_data[ArrayMesh.ARRAY_VERTEX]
	
	#TODO: You fucked up again. Find out where, and fix.
	# Iterate through array_index to find the provided vertex
	for i in array_index.size():
		if array_index[i] == vertex:
			# Find the first vertex in the triangle (set of 3) it belongs to
			var first_tri = i - i % 3
			# Append the normal of the triangle face
			face_normals.append(get_face_normal(
					array_vertex[first_tri],
					array_vertex[first_tri+1],
					array_vertex[first_tri+2]))
			# Jump to the first vertex in the next triangle
			if first_tri+3 < array_index.size():
				i = first_tri+3
			else: break
...
func get_face_normal(a:Vector3, b:Vector3, c:Vector3):
	var side1 = b - a
	var side2 = c - a
...
func get_normalized_average(vectors:PackedVector3Array):
